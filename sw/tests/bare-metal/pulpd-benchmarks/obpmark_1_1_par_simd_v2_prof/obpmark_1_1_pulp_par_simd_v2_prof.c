...
/* v2: partial vectorization. The 4-frame sum overflows 16 bit
 * (4*65534 > 65535), so we keep SIMD only for the loads (4 v2u loads instead
 * of 8 scalar loads) and do the accumulation + threshold in 32-bit scalar. */
static void f_scrub_range(frame16_t *frame, frame16_t *fs, unsigned int frame_i,
    ...
    for (y = 0; y < frame->h; y++) {
        ...
        for (x = xs; x + 1u < xe; x += 2u) {
            ...
            v2u c = *(v2u *)&fp1[i];
            v2u d = *(v2u *)&fp2[i];
            uint32_t sum0 = (uint32_t)a[0] + (uint32_t)b[0] + (uint32_t)c[0] + (uint32_t)d[0];
            uint32_t sum1 = (uint32_t)a[1] + (uint32_t)b[1] + (uint32_t)c[1] + (uint32_t)d[1];
            uint32_t mean0 = sum0 >> 2;
            uint32_t mean1 = sum1 >> 2;
            if (fcur[i]      > (mean0 << 1)) fcur[i]      = (uint16_t)mean0;
            if (fcur[i + 1u] > (mean1 << 1)) fcur[i + 1u] = (uint16_t)mean1;
        }
        if (x < xe) {
            unsigned int i = row + x;
...
testcase_t testcases[] = {
    { .name = "obpmark_1_1_par_simd_v2_prof", .test = check_obpmark },
    {0, 0}
};
...
void check_obpmark(testresult_t *result, void (*start)(), void (*stop)()) {
    ...
    printf("PULP cluster parallel + SIMD16 v2 + PROFILING (%u cores, RV32) - Astral @ 50 MHz\n", num_cores);
