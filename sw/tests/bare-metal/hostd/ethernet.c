...
#define IRQID 19 // index of ethernet irq in the irq vector input to the PLIC

#define CHS_ETHERNET_BASE_ADDR 0x0300c000
...
int main(void) {
  ...
  *reg32(CHS_ETHERNET_BASE_ADDR, MACLO_OFFSET)          = 0x98001032;
  // High 16 bit Mac Address
  *reg32(CHS_ETHERNET_BASE_ADDR, MACHI_OFFSET)          = 0x00002070;
  // DMA Source Address
  *reg32(CHS_ETHERNET_BASE_ADDR, IDMA_SRC_ADDR_OFFSET)  = &__base_dram;
  // DMA Destination Address
  *reg32(CHS_ETHERNET_BASE_ADDR, IDMA_DST_ADDR_OFFSET)  = 0x0;
  // Data length
  *reg32(CHS_ETHERNET_BASE_ADDR, IDMA_LENGTH_OFFSET)    = 0x40;
  // Source Protocol
  *reg32(CHS_ETHERNET_BASE_ADDR, IDMA_SRC_PROTO_OFFSET) = 0x0;
  // Destination Protocol
  *reg32(CHS_ETHERNET_BASE_ADDR, IDMA_DST_PROTO_OFFSET) = 0x5;
  // Validate Request to DMA
  *reg32(CHS_ETHERNET_BASE_ADDR, IDMA_REQ_VALID_OFFSET) = 0x1;
  // Stop accepting new request
  *reg32(CHS_ETHERNET_BASE_ADDR, IDMA_REQ_VALID_OFFSET) = 0x0;
  // Enable DMA to move data
  *reg32(CHS_ETHERNET_BASE_ADDR, IDMA_RSP_READY_OFFSET) = 0x1;
